...
function promo(){
    let promocode = $('#kafe').val();
    console.log(promocode);
    

    // for (i = 0; i< promo1.length; i++){
    //    if (promocode.value == promo1[i].promoname) {
    //        return true;
    //    }
...
//Ordering: print out a list of item + destination
function buyItem() {
    let cartItem = localStorage.getItem('productsInCart');
    cartItem = JSON.parse(cartItem);
    ...
    if(cartItem && showCart && totalPrice){
        ...
        Object.values(cartItem).map(item =>{
            print.innerHTML += `
                <div id = "printCart">
                    <p>${item.title}</p>
                    <p>Price: ${item.price},000vnd</p>
                    <p> Quantity: ${item.quantity}</p>
                </div>
            `
            totalPrice.innerHTML += `
            <p id="promo-code"></p>
            <p id="total">${item.cartcost},000 vnd</p>
            <p id"location"> </p>
        `
